...
class Tx():
    ...
    def load_gain_index_from_ram(self, devs, pol):
        ...
        if not isinstance(devs,list):
            ...
            index_list = [0] * len(devs)
        i = 0
        for dev in devs:
            if pol == 'V':
                index_list[i] = (self._spi.rd(dev, RAM_ADDR+V_OFFSET, 10) & 0xffc00000000000000000) >> 70
                i = i + 1
            if pol == 'H':
                index_list[i] = (self._spi.rd(dev, RAM_ADDR+H_OFFSET, 10) & 0xffc00000000000000000) >> 70
                i = i + 1
        return index_list
