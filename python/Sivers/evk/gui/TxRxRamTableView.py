...
import evk.gui.RamFileSelector
...
class TxRxRamTableView():
    def __init__(self, parent, gui_handler, dev, txrx_ram, *args, **kwargs):
        ...
        if txrx_ram.lower() == 'tx_ram':
            self.spinbox_selected_index.set(self.gui_handler.host.chip.tx.load_gain_index_from_ram(dev, 'H')[0])
        else:
        ...
        self.fv = TxRxRamTableFieldView(self, parent, self.gui_handler, dev, txrx_ram, self.ram_value, 10, self.value_modified, *args, **kwargs)
        self.bv = TxRxRamTableByteView(self, parent, self.gui_handler, dev, txrx_ram, self.ram_value, col_width=8, value_changed_callback=self.value_modified, *args, **kwargs)
        self.cv = TxRxRamTableCompactView(self, parent, self.gui_handler, dev, txrx_ram, self.ram_value, value_changed_callback=self.value_modified, *args, **kwargs)
    ...
    def polarization_selected(self, event):
        ...
        if self.txrx_ram.lower() == 'tx_ram':
            self.spinbox_selected_index.set(self.gui_handler.host.chip.tx.load_gain_index_from_ram(self.dev, p.upper())[0])
        else:
    ...
    def open_table_selector(self):
        ...
        if filename != None:
            ...
            ram_file_selector = evk.gui.RamFileSelector.RamFileSelector(self.parent, filename.name, filter=filter, select_callback=self.ram_table_selected)
    ...
    def ram_table_selected(self, filename, table_id):
        if self.get_selected_polarization() == 'v' and self.txrx_ram == 'RX_RAM':
            ...
            tx_v_gain_index = self.gui_handler.host.chip.tx.load_gain_index_from_ram(self.dev, 'V')[0]
            tx_h_gain_index = self.gui_handler.host.chip.tx.load_gain_index_from_ram(self.dev, 'H')[0]
...
class TxRxRamTableCompactView():
    def __init__(self, owner, parent, gui_handler, dev, txrx_ram, ram_value, col_width=15, value_changed_callback=None, *args, **kwargs):
        self.owner = owner
        self.parent = parent
        self.gui_handler = gui_handler
        self.dev = dev
        ...
        if self.pointed_column != column:
            ...
            if text.endswith('_v'):
                if self.polarization == 'v':
                    ...
                    text = text[0:-1] + 'h'
            self.toolTip = evk.gui.tooltip.ToolTip(self.main_table, text=text+'['+str(self.field_sizes[int(column.replace('#',''))-1])+' bits]')
