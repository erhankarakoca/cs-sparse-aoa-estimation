...
class FieldView():
    ...
    def read_reg_value(self):
        if 'ram' in self.reg:
            current_value = (self.host.gui_handler.gd[self.dev.get_name()].read_ram(self.reg, self.owner.row) & self.mask) >> self.host.spi.register_map.reg_map[self.reg][self.field]['Lsb']
        else:
            current_value = (self.host.gui_handler.gd[self.dev.get_name()].read_register(self.reg) & self.mask) >> self.host.spi.register_map.reg_map[self.reg][self.field]['Lsb']
        self.field_value.set(fhex(current_value, self.num_of_digits))
