...
import evk.gui.actions.synth_actions as synth_actions
import evk.gui.actions.general_actions as general_actions


class GeneralView():

    def __init__(self, parent, gui_handler, dev, main_frame, *args, **kwargs):
    ...
    def _state_set(self):
        ...
        if not state == '':
            ga = general_actions.GeneralActions(self.host)
            ga.state_set(self.dev, state)
            self.state_set_button.configure(fg='green')

    def _init(self, block):
        ga = general_actions.GeneralActions(self.host)
        ga.init(self.dev, block)
        if block == 'CHIP':
    ...
    def _pwr(self, port):
        ga = general_actions.GeneralActions(self.host)
        status = ga.pwr_status(port)
        if status:
            status = ga.pwr_off(port)
        else:
            status = ga.pwr_on(port)
        if status:
    ...
    def _misc(self, port):
        ga = general_actions.GeneralActions(self.host)
        status = ga.misc_status(port)
        if status:
            status = ga.misc_off(port)
        else:
            status = ga.misc_on(port)
        if status:
    ...
    def setup_set(self):
        ...
        vcxo_freq = sa.setup_set(self.dev, freq_rff, frac_mode, sd_order)
        self.ref_freq.set(round(vcxo_freq, 2))

        actual_rf_freq = sa.get_rf_freq(self.dev)
