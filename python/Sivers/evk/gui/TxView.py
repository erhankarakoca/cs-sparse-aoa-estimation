...
import evk.gui.actions.tx_actions as tx_actions
...
class TxView():
    ...
    def _beam_set(self):
        ...
        ta = tx_actions.TxActions(self.host)
        ta.beam(self.dev, pol, beam_index)
    ...
    def _rf_gain_set(self):
        ...
        if rf_gain_index != None:
            ...
            ta = tx_actions.TxActions(self.host)
            ta.rf_gain(self.dev, gain_pol, rf_gain_index)
    ...
    def setup_set(self):
        ...
        ta = tx_actions.TxActions(self.host)
        ta.setup(self.dev, mode, pol, latitude, ant_en_v, ant_en_h)
        self.setup_set_button.configure(fg='green')

    def busy_detect(self):
        if self.main_view.busy:
            self.main_view.top.wm_attributes('-disabled', 'True')
            self.main_view.top.after(2000, lambda: self.busy_detect())
        else:
            self.main_view.top.wm_attributes('-disabled', 'False')
    ...
    def _calibrate(self):
        ...
        self.ta = tx_actions.TxActions(self.host)
        self.host.gui_handler.TH.put(lambda : self.main_view.set_busy(True))
        self.host.gui_handler.TH.put(lambda : self.ta.dco_calibrate(self.dev, mode, tx_pol, cross_pol))
        self.host.gui_handler.TH.put(lambda : self.main_view.set_busy(False))
        self.main_view.top.after(1000, lambda: self.busy_detect())
