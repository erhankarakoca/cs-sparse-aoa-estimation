...
class GainView():
    ...
    def __init__(self, parent, gui_handler, dev, mode, *args, **kwargs):
    ...
    def _add_gain_adjust_frame(self):
        ...
        if self.mode == 'RX':
            self.selected_gain_index_v = tk.IntVar(value=self.host.spi.rd(self.dev, 'rx_gain_indx_v'))
        else:
            self.selected_gain_index_v = tk.IntVar(value=self.gui_handler.host.chip.tx.load_gain_index_from_ram(self.dev, 'V')[0])
        ...
        if self.mode == 'RX':
            self.selected_gain_index_h = tk.IntVar(value=self.host.spi.rd(self.dev, 'rx_gain_indx_h'))
        else:
            self.selected_gain_index_h = tk.IntVar(value=self.gui_handler.host.chip.tx.load_gain_index_from_ram(self.dev, 'H')[0])
        ...
        self.ram_v_regfield = evk.gui.RegFieldView.RegFieldView(bottom_left_frame, self._ram_v_value_changed_externally, self.host, self.dev, self.mode.lower()+'_ram_v', wr_button_text='Write & Sync', rd_button_text=None)
        self.ram_v_regfield.row = self.selected_gain_index_v.get()
        self.ram_h_regfield = evk.gui.RegFieldView.RegFieldView(bottom_right_frame, self._ram_h_value_changed_externally, self.host, self.dev, self.mode.lower()+'_ram_h', wr_button_text='Write & Sync', rd_button_text=None)
    ...
    def _ram_h_value_changed_externally(self):
        if self.mode == 'RX':
            ...
            self.host.chip.tx.gain_rf(self.dev, self.selected_tx_gain_index_h.get(), 'TH')
